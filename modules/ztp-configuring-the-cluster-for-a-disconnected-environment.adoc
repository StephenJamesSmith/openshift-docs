...
. The AgentServiceConfig should look like this:
+
[source,yaml]
----
...
. For disconnected installs, you need to deploy an NTP clock that is reachable through the disconnected network.
You can do this by configuring chrony to act as server, editing `/etc/chrony.conf`, and adding the following allowed ipv6 range:
+
[source,yaml]
----
