...
Optionally, after you have created the AgentClusterInstall custom resource, you can configure static IP addresses for the managed clusters.
...
----
<1> `mac-address` is the MAC address of the target bare metal machine (the same one used in the `BareMetalHost` resource).
<2> `ip-address` is the static IP of the target bare metal machine.
...
<6> `name` must match the name listed under `interfaces`.
<7> `mac-address` must match the mac address listed under `interfaces`.
