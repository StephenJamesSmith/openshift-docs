...
. Use the `describe` command to provide an in-depth description of the agent’s condition.
Statuses to be aware of include `BackendError`, `InputError`, `ValidationsFailing`, `InstallationFailed`, and `AgentIsConnected`.
+
[source,terminal]
----
...
. Check the status of the managed cluster’s addon services:
+
[source,terminal]
----
